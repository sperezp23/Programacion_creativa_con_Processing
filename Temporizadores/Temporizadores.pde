...
color
temperatura_color, // Variable para controlar el cambio de color del circulo. 
...
void draw(){
  ...
  aceleracion = tiempo_actual * 0.5e-3; // Calcular la 'aceleración' del circulo.
  ...
  temperatura = norm(aceleracion, 0, 20); // Pasar la aceleración al intervalo [0,1] para convertirla en 'temperatura'.
  temperatura_color = lerpColor(color_inicial, color_final, temperatura); // Calcular el color correspondiente a cada temperatura.
  ...
  fill(temperatura_color); // Rellenar las figuras del color indicado.
